...
def build_district_centroids() -> pd.DataFrame:
    ...
    gj = load_geojson(URL_GEO_DISTRICTS)
    rows = []
    for feat in gj.get("features", []):
        ...
        if not (dist and prov and dep):
            continue
        c = _centroid(feat.get("geometry", {}).get("coordinates"))
        if c is None:
            continue
        lat, lng = c
        rows.append(
...
def build_region_centroids() -> pd.DataFrame:
    gj = load_geojson(URL_GEO_REGIONS)
    rows = []
    for feat in gj.get("features", []):
        props = feat.get("properties", {})
...
st.title("🗺️ Dashboard Empadronadores Perú")
st.caption("Cobertura municipal y estado de empadronadores · refresco cada 5 min")
...
# Cargar geometrías
try:
    geo_regions = load_geojson(URL_GEO_REGIONS)
...
with g4:
    ...
    fig.update_layout(
        barmode="stack", height=420, margin=dict(l=10, r=10, t=40, b=10)
    )
    st.plotly_chart(fig, use_container_width=True)
